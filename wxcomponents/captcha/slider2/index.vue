<template>
<uni-shadow-root class="captcha-slider2-index"><view class="tianai-captcha-slider2">
    <view class="tip">
    ...
    <view class="content">
        ...
        <view class="slider-img-div" :style="'left: '+(sliderMove2)+'px;transform: rotate('+(rotateDeg)+'deg);transform-origin: right '+(topOrBottom)+';'">
...
global['__wxRoute'] = 'captcha/slider2/index'
...
Component({
  ...
  methods: {
    onBgImageLoad(e) {
      ...
      let attrs = [];
      attrs[0] = (300 - 180 - 55) / 300;
      attrs[1] = this.properties.captchaData.data.attr;
      attrs[2] = 300;
      this.setData({
        attrs: attrs,
        topOrBottom: attrs[1] < 0 ? "top" : "bottom"
      })
    },
    touchstart(e) {
      let requestData = {
    ...
    touchmove(e) {
      ...
      if (moveX > 0 && moveX < this.data.endX) {
        this.setData({
          sliderMove: moveX,
          sliderMove2: moveX * this.data.attrs[0],
          rotateDeg: this.data.attrs[1] * ((moveX * this.data.attrs[0]) / this.data.attrs[2])
        })
      }
    },
    touchend(e) {
...
export default global['__wxComponents']['captcha/slider2/index']
</script>
<style platform="mp-weixin">
/* components/captcha/slider/index.css */
 .tianai-captcha-slider2 {
  z-index: 999;
  position: absolute;
  left: 0;
  top: 0;
...
.tianai-captcha-slider2 .slider-tip {
  margin-bottom: 5px;
  font-weight: bold;
  font-size: 15px;
  line-height: normal;
...
.tianai-captcha-slider2 .tips.tips-error {
  background-color: #FF5D39;
}
.tianai-captcha-slider2 .tips.tips-success {
  background-color: #39C522;
}
.tianai-captcha-slider2 .tips.tips-on {
  bottom: 0;
}

.tianai-captcha-slider2 .move-track-font {
  font-size: 15px;
}

.tianai-captcha-slider2 .content {
  width: 100%;
  height: 180px;
  position: relative;
  overflow: hidden;
...
.tianai-captcha-slider2 .content .bg-img-div{
  width: 100%;
  height: 100%;
  position: absolute;
  transform: translate(0px, 0px);
}
.tianai-captcha-slider2 .content .bg-img-div .bg-img {
  height: 100%;
  width: 100%;
  border-radius: 5px;
}
.tianai-captcha-slider2 .content .bg-img-div .bg-div {
  position: absolute;
  left: 0;
  top: 0;
  width: 100%;
...
.tianai-captcha-slider2 .content .bg-img-div .bg-div .bg-div-slice {
  position: absolute;
}

.tianai-captcha-slider2 .content .slider-img-div {
  height: 100%;
  position: absolute;
  transform: translate(0px, 0px);
  width: 55px;
...
.tianai-captcha-slider2 .content .slider-img-div .slider-move-img {
  height: 100%;
  width: 100%;
}

.tianai-captcha-slider2 .slider-move {
  line-height: 38px;
  font-size: 14px;
  text-align: center;
  white-space: nowrap;
...
.tianai-captcha-slider2 .slider-move .slider-move-track {
  position: relative;
  height: 32px;
  line-height: 32px;
  text-align: center;
...
.tianai-captcha-slider2 .slider-move .slider-move-track .slider-move-track-mask {
  border-color: rgb(2, 152, 248);
  background-color: rgb(137, 210, 255);
  height: 100%;
}

.tianai-captcha-slider2 .slider-move .slider-move-track .slider-move-btn {
